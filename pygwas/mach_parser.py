...
class Parser(DataParser):
    ...
    def load_family_details(self, pheno_covar):
        ...
        if DataParser.compressed_pedigree:
            #data, serr = sys_call('gunzip -c %s | head -1 | wc -w' % (file))
            #self.col_count = int(data[0].strip())
            ...
            #data, serr = sys_call('head -1 %s | wc -w' % (file))
            #self.col_count = int(data[0])
    ...
    def parse_genotypes_(self, lb, ub):
        dosages = numpy.empty((self.ind_count, ub-lb), dtype=str)
        idx = 0
        #print >> sys.stderr, "Dosages Shape: ", dosages.shape
        for line in self.openfile(self.current_file):
            words = line.strip().split()[lb:ub]
            if len(words) < 1:
                raise EOFError

            # Dosages are given for the referent allele, so we need to flip them
            dosages[idx] = 2.0 - numpy.array(words)
            idx += 1
        return dosages


    def load_genotypes(self):
        """Due to memory issues, we'll load the dosages in chunks and iterate over the chunks until we
           run out and then load the next chunk up until we have visited them all.
        """
